...
AGENTDOCK_ACP_COMMAND=/absolute/path/to/executable
...
3. Poll `events` with `after_seq=0`, then pass each returned `next_seq` unchanged into the next call.
4. Confirm event sequence numbers are monotonic and no event is skipped across pages.
5. Confirm `first_seq` and `truncated` make bounded-ring eviction explicit during a high-volume synthetic or staging-only run.
6. Confirm terminal status and terminal event agree.
7. Confirm complete model transcripts and event streams are not written to AgentDock's session store.
...
4. If steering is advertised, steer a running prompt and verify the adapter response.
5. Exercise the configured global prompt limit across separate sessions.
6. Confirm a second concurrent prompt on the same session is rejected.
...
5. In a disposable instance, race close/delete against prompt completion and verify late finalization cannot recreate deleted state or overwrite closed state.
6. Confirm pending interactions expire or cancel during shutdown.
