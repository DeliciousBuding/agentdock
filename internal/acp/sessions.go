...
func (m *Manager) NewSession(ctx context.Context, cwd string, additionalDirectories []string) (SessionResult, error) {
	...
	record, err := m.persistNewSession(response.SessionID, resolved, additional)
	if err != nil {
		return SessionResult{}, err
	}
...
func (m *Manager) LoadSession(ctx context.Context, id string) (SessionResult, error) {
	...
	record, err = m.markSessionReady(record)
	if err != nil {
		return SessionResult{}, err
	}
	return SessionResult{Session: record, Modes: response.Modes, ConfigOptions: response.ConfigOptions, Agent: process.initialize.AgentInfo}, nil
...
func (m *Manager) CloseSession(ctx context.Context, id string) (SessionRecord, error) {
	...
	previousTerminal, hadPreviousTerminal, err := m.beginTerminalTransition(id, SessionClosed)
	if err != nil {
		return SessionRecord{}, err
	}
	succeeded := false
...
func (m *Manager) DeleteSession(ctx context.Context, id string) error {
	...
	previousTerminal, hadPreviousTerminal, err := m.beginTerminalTransition(id, sessionDeleted)
	if err != nil {
		return err
	}
	succeeded := false
...
func (m *Manager) persistNewSession(remoteID, cwd string, additional []string) (SessionRecord, error) {
	id, err := newID("acps")
	if err != nil {
		return SessionRecord{}, err
	}
	...
	record := SessionRecord{
		...
		RemoteSessionID: remoteID, CWD: cwd, AdditionalDirectories: append([]string(nil), additional...),
	...
	if existing := m.remoteToLocal[remoteID]; existing != "" {
		m.mu.Unlock()
...
func (m *Manager) markSessionReady(record SessionRecord) (SessionRecord, error) {
...
func (m *Manager) beginTerminalTransition(id string, target SessionStatus) (SessionStatus, bool, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	if m.closed {
