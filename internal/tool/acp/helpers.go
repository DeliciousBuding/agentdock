...
func acpToolError(err error) error {
	...
	if errors.As(err, &acpErr) {
		...
		case strings.Contains(acpErr.Code, "INVALID"), strings.Contains(acpErr.Code, "DENIED"), strings.Contains(acpErr.Code, "NOT_FOUND"), strings.Contains(acpErr.Code, "CLOSED"), strings.Contains(acpErr.Code, "SETTLED"), strings.Contains(acpErr.Code, "MISMATCH"):
			category = "validation"
		}
