...
func TestNormalizeACPProfileCanonicalizesRoots(t *testing.T) {
	...
	cfg := Config{
		...
		ACPAllowedRoots: []string{root, filepath.Join(root, ".")},
	}
...
func TestNormalizeACPRejectsUnsafeConfiguration(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "invalid agent name", mutate: func(cfg *Config) {
			cfg.ACPAgentName = "bad\nname"
			cfg.ACPCommand, _ = os.Executable()
			cfg.ACPAllowedRoots = []string{t.TempDir()}
		}},
		{name: "relative command", mutate: func(cfg *Config) { cfg.ACPCommand = "agent"; cfg.ACPAllowedRoots = []string{t.TempDir()} }},
		{name: "missing roots", mutate: func(cfg *Config) { cfg.ACPCommand, _ = os.Executable() }},
		{name: "relative root", mutate: func(cfg *Config) { cfg.ACPCommand, _ = os.Executable(); cfg.ACPAllowedRoots = []string{"relative"} }},
		{name: "filesystem root", mutate: func(cfg *Config) { cfg.ACPCommand, _ = os.Executable(); cfg.ACPAllowedRoots = []string{filesystemRoot} }},
		{name: "invalid direct env mapping", mutate: func(cfg *Config) {
			cfg.ACPCommand, _ = os.Executable()
			cfg.ACPAllowedRoots = []string{t.TempDir()}
			cfg.ACPEnvFromEnv = map[string]string{"BAD-NAME": "HOST_KEY"}
		}},
		{name: "too many direct args", mutate: func(cfg *Config) {
			cfg.ACPCommand, _ = os.Executable()
			cfg.ACPAllowedRoots = []string{t.TempDir()}
			cfg.ACPArgs = make([]string, 129)
		}},
		{name: "too many prompts", mutate: func(cfg *Config) {
			cfg.ACPCommand, _ = os.Executable()
			cfg.ACPAllowedRoots = []string{t.TempDir()}
			cfg.ACPMaxPrompts = 9
		}},
